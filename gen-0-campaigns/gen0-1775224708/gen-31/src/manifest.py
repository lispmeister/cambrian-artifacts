"""Manifest building, hash computation, and related utilities."""
...
def compute_artifact_hash(artifact_root: Path, files: list[str]) -> str:
    ...
    Files are processed in lexicographic order. Each file contributes:
    - its relative path (UTF-8 encoded)
    - a null byte separator
    - its content (raw bytes)
    """
...
def extract_contracts_from_spec(spec_content: str) -> list[dict[str, Any]] | None:
    ...
    """
    # Look for ```contracts ... ``` fenced block
...
def build_manifest(
    ...
    # Extract contracts from spec if available
